...
describe('CampaignService', () => {
  ...
  const clients = [
    { phone: '5491111111111', name: 'Ana', company: 'ACME', systems: ['crm'], tags: ['cliente-activo'] },
    { phone: '5492222222222', name: 'Luis', company: 'ACME', systems: ['erp'], tags: ['prospecto'] },
  ];
  ...
  it('creates preview jobs for dry runs', async () => {
    const run = await service.createRun('welcome', undefined, true);

    expect(run.status).toBe('dry_run');
    expect(run.jobs.every((job: any) => job.status === 'preview')).toBe(true);
  });

  it('sends the next queued job and recomputes totals', async () => {
    const created = await service.createRun('welcome');
    const run = await service.processNextQueuedJob(created.id);

    expect(whatsAppAdapter.sendBroadcast).toHaveBeenCalledWith('5491111111111@c.us', 'Hola Ana');
  ...
  it('requeues failed jobs with backoff before final failure', async () => {
    ...
    const created = await service.createRun('welcome');
    const run = await service.processNextQueuedJob(created.id);
    const failedJob = run.jobs.find((job: any) => job.phone === '5491111111111');
  ...
  it('segments campaign audiences by tags', async () => {
    ...
    const run = await service.createRun('tagged');
